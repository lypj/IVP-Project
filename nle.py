...
def nlePCA(img, patchsize=7, conf=(1-1e-6), itr=3):
	...
	DD = Dh.T@Dh + Dv.T@Dv
	r = torch.matrix_rank(DD, symmetric=True)
	...
	for cha in range(1,img.shape[1]+1):
		...
		if X.shape[1] < X.shape[0]:
			...
			d = torch.symeig(cov)[0] # covariance mat eigenvalues are real
			sig2 = d[0]
